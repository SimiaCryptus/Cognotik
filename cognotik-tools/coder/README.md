# Code Research
  ...
  follow-ups, patches, raw JSON), manage research history, and configure model
  selections. It works either inside a hosted docops session or standalone —
  in direct-file mode the base path is the directory the page itself is served
  ...
  - **Working directory** – where `<slug>/` document folders are read/written
    (default `queries`).
  - **Ops directory** – where the `*.op.md` files above live (default `ops`).
