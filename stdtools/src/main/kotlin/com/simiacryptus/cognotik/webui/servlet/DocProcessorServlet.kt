  ...
  protected open fun resolveRoot(
    ...
    when {
      null == sessionOwner -> {
        ...
        writeError(response, HttpServletResponse.SC_NOT_FOUND, "Session not found: $session")
        return null
      }

      sessionOwner != user.id -> {
        log.info("User '${user.id}' attempted to access session '$session' owned by '$sessionOwner'")
        writeError(
          response, HttpServletResponse.SC_FORBIDDEN,
