        ...
        while (outerChanged && passes < 5) {
          ...
          while (changed) {
            ...
            fun rejectionReason(element: Element): String? = when {
              ...
              element.meaningfulAttributeCount() != 0 -> "has-attributes(${
                element.attributes().size()
              }:${element.attributes().asList().joinToString { it.key }})"

              element.children().size != 1 -> "child-element-count=${element.children().size}"
              element.parent() == null -> "no-parent"
              !element.textNodes().all { it.text().isBlank() } -> "has-nonblank-text-nodes(${
                element.textNodes().filter { it.text().isNotBlank() }.joinToString("|") { "'${it.text().take(20)}'" }
              })"
