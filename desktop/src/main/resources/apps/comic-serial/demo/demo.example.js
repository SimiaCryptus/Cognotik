...
async function waitForBadgeDone(page, badgeId, label, timeout = 300000) {
    ...
    console.log(`✅ ${label} badge is done.`);
     await diagnosticSnapshot(page, `badge-done-${badgeId}`);
...
test('Philosophical Calculator Demo', async ({browser}) => {
    ...
    try {
        ...
        await page.goto(HUB_URL, {waitUntil: 'networkidle'});
         await diagnosticSnapshot(page, 'step0-hub-loaded');
        ...
        await handleLogin(page, CREDENTIALS_PATH, NARRATION, {
         ...
         await diagnosticSnapshot(page, 'step0-post-login');
        ...
        await page.waitForTimeout(MEDIUM_PAUSE);
         await diagnosticSnapshot(page, 'step1-app-launched');
        ...
        // -----------------------------------------------------------------
         await diagnosticSnapshot(page, 'step-app-specific-placeholder');

        await say(page, NARRATION['OUTRO'], LONG_PAUSE);
        await page.waitForTimeout(LONG_PAUSE);
         await diagnosticSnapshot(page, 'final-outro-complete');
        console.log('\n✅ Demo complete.');
    } catch (error) {
        console.error('❌ Demo failed with error:', error);
         await diagnosticSnapshot(page, 'error-state').catch(e =>
             console.error('⚠️  Failed to capture error diagnostic snapshot:', e)
         );
        throw error;
    } finally {
