(function () {
    ...
    var currentPlatform = 'sh'; // 'sh' or 'cmd'
    ...
    function updatePlatformUI() {
        ...
        var runOp = getRunOp();
        // Update script filename display
    ...
    // === Path Normalization ===
    // Server may return paths with backslashes on Windows; normalize to forward slashes
    ...
    async function waitForTask(targetPath, maxWaitMs) {
        ...
        while (Date.now() - startTime < maxWait) {
            ...
            if (statusData && statusData.tasks) {
                // Find the task by normalized path (server may use backslashes on Windows)
                var task = null;
                for (var key in statusData.tasks) {
                    if (statusData.tasks.hasOwnProperty(key) && normalizePath(key) === normalizePath(targetPath)) {
                        task = statusData.tasks[key];
                        break;
                    }
                }
                if (task) {
                    if (task.status === 'COMPLETED') {
                        completedTargets[normalizePath(targetPath)] = true;
                        return task;
                    }
        ...
        var badgeMap = {};
        badgeMap[getScriptFilename()] = 'badge-codegen';
        badgeMap['code/fix_log.md'] = 'badge-run';

        var stageMap = {};
        stageMap[getScriptFilename()] = 'stage-codegen-status';
        stageMap['code/fix_log.md'] = 'stage-run-status';
        // Track targets that have been definitively completed by runSequential/waitForTask
        // so that pollStatus doesn't regress them back to 'running' due to stale server data
        ...
        async function pollStatus() {
            ...
            for (var target in statusData.tasks) {
                ...
                // Don't regress targets that we've already confirmed as completed
                ...
                updateSessionLinks(normalizedTarget, taskInfo);
            }
        }
        ...
        function updateSessionLinks(target, taskInfo) {
            ...
            var normalizedTarget = normalizePath(target);
            var safeTarget = normalizedTarget.replace(/[^a-zA-Z0-9]/g, '-');
            ...
            if (!container) {
                ...
                // Try to insert near the relevant step card
                var viewerIds = {
                    'code/fix_log.md': 'card-step-2'
                };
                viewerIds[getScriptFilename()] = 'card-step-1';
                var cardId = viewerIds[target];
                var card = cardId ? document.getElementById(cardId) : null;
                if (card) {
                    // Insert the session link container inside the card, at the end of the step div
                    var stepDiv = card.querySelector('.step');
                    if (stepDiv) {
                        // Insert before the viewer(s) so it appears between buttons and viewer
                        var firstViewer = stepDiv.querySelector('.viewer');
                        if (firstViewer) {
                            stepDiv.insertBefore(container, firstViewer);
                        } else {
                    ...
                    // Fallback: append to pipeline section
                    var pipelineSection = document.getElementById('section-pipeline');
                    if (pipelineSection) {
                        pipelineSection.appendChild(container);
                    }
                }
            }
            if (!container) return;
            ...
            btn.addEventListener('click', function () {
                ...
                if (mode === 'generate') {
                    ...
                    if (scriptEditor && !scriptEditor.value.trim()) {
                        readFile(getScriptFilename()).then(function (content) {
        ...
        document.getElementById('save-script-manual').addEventListener('click', async function () {
            ...
            try {
                this.disabled = true;
                await writeFile(getScriptFilename(), content);
        ...
        document.getElementById('load-script-into-editor').addEventListener('click', async function () {
            try {
                this.disabled = true;
                var content = await readFile(getScriptFilename());
                if (content !== null && content.trim().length > 0) {
                    scriptEditor.value = content;
        ...
        document.getElementById('reload-models').addEventListener('click', function () {
            this.disabled = true;
            var self = this;
            loadApiProviders().finally(function () {
                self.disabled = false;
            });
        });
        ...
        document.querySelectorAll('.btn-run').forEach(function (btn) {
            btn.addEventListener('click', async function () {
                ...
                try {
                    ...
                    // Update pipeline stage diagram
                    var btnStageMap = {'code/script.sh': 'codegen', 'code/fix_log.md': 'run'};
        ...
        document.getElementById('run-all').addEventListener('click', async function () {
            ...
            try {
                ...
                if (isManualMode) {
                    ...
                    if (manualScript) {
                        await writeFile(getScriptFilename(), manualScript);
                ...
                steps.push({
                    ...
                    afterFn: async function () {
                        ...
                        try {
                            var scriptContent = await readFile(getScriptFilename());
                            if (scriptContent) {
        ...
        async function checkExistingFiles() {
            ...
            if (statusData && statusData.tasks) {
                for (var target in statusData.tasks) {
                    ...
                    // Also check the other platform's script file for status
            ...
            try {
                var scriptExists = await readFile(getScriptFilename());
        ...
        async function loadUsageData() {
            ...
            var modelList = Object.keys(allModels).map(function (k) {
                return allModels[k];
            });
        ...
        // === Initialize ===
        // Auto-detect platform if no saved preference
        if (!savedPlatform) {
