...
DataIngest - Iteratively parse unstructured logs/text into structured data
  ...
  ** Efficiently handles large files via streaming extraction
  ...
  override fun run(
    ...
    task.pool.submit {
      try {
        ...
        // Save Patterns
        patternsFile?.writeText((registry.toJson()))
        transcript?.write(
          """
                ### Discovered Patterns
                <details>
        ...
        // Generate a standalone Node.js script that replicates the discovered extraction rules
        val (scriptFileLink, scriptFile) = task.createFile("process.js")
        scriptFile?.writeText(buildNodeScript(registry, allFields))
        log("📜 Generated Node.js reprocessing script: `process.js`")
        ...
        val summary = buildString {
          ...
          appendLine("- [Node.js Reprocessing Script]($scriptFileLink)")
          appendLine()
        ...
        task.complete()
        resultFn("Successfully ingested $totalExtracted records from ${files.size} files using ${registry.size} patterns. Artifacts: data.jsonl, data.csv, patterns.json, index.csv, process.js.")
  ...
  private fun buildNodeScript(registry: List<PatternRegistryItem>, allFields: List<String>): String {
    val patternsJson = registry.toJson()
    val allFieldsJson = allFields.toJson()
    return """
#!/usr/bin/env node
/**
* Auto-generated by DataIngestTask.
* Applies the discovered regex patterns to new input files using the
* same extraction rules used during the original ingestion run.
*
* Usage:
*   node process.js <file1> <file2> ... [--out-dir <dir>]
*
* Outputs (in the current working directory unless --out-dir is specified):
*   data.jsonl, data.csv, index.csv
*/
'use strict';
const fs = require('fs');
...
const ALL_FIELDS = $allFieldsJson;
...
async function processFile(filePath, compiledPatterns, streams, state) {
  ...
  for await (const line of rl) {
    ...
    const lineLength = lineBytes + 1; // +1 for newline approximation
    for (const pattern of compiledPatterns) {
      const match = pattern.regex.exec(line);
      if (match) {
        const recordId = crypto.randomUUID();
        const dataMap = { timestamp: new Date().toISOString() };
        for (const field of pattern.fields) {
          const value = match.groups ? match.groups[field] : undefined;
          if (value !== undefined) {
            dataMap[field] = value;
          }
        }
        const json = JSON.stringify(dataMap);
        const jsonBytes = Buffer.byteLength(json + '\n', 'utf8');
        streams.data.write(json + '\n');
        const csvRow = ALL_FIELDS.map(f => csvEscape(dataMap[f])).join(',');
        streams.csv.write(csvRow + '\n');
        const indexRow = [
          recordId,
          pattern.id,
          fileName,
        ...
        streams.index.write(indexRow + '\n');
        state.totalBytes += jsonBytes;
...
async function main() {
  ...
  if (files.length === 0) {
    console.error('Usage: node process.js <file1> <file2> ... [--out-dir <dir>]');
    process.exit(1);
  }
  ...
  const streams = {
    data: fs.createWriteStream(path.join(outDir, 'data.jsonl')),
    csv: fs.createWriteStream(path.join(outDir, 'data.csv')),
    index: fs.createWriteStream(path.join(outDir, 'index.csv'))
  };
  streams.csv.write(ALL_FIELDS.join(',') + '\n');
  streams.index.write('message_id,pattern_id,source_file,source_offset_start,source_offset_end,data_offset\n');
  const state = { totalBytes: 0, totalExtracted: 0 };
  for (const file of files) {
    await processFile(file, compiledPatterns, streams, state);
  }
  await Promise.all([streams.data, streams.csv, streams.index].map(s => new Promise((resolve) => s.end(resolve))));
  console.log('Extracted ' + state.totalExtracted + ' records from ' + files.length + ' files using ' + compiledPatterns.length + ' patterns.');
}
main().catch(err => {
