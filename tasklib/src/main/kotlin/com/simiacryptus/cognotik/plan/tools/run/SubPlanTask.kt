...
class SubPlanTask(
  ...
  override fun run(
    ...
    try {
      ...
      val cognitiveMode: CognitiveModeType<*> = (typeConfig.cognitiveMode?.newSettings() ?: orchestrationConfig.cognitiveSettings)?.type
        ?: throw IllegalArgumentException("No cognitive mode specified for SubPlanningTask. Please set a cognitive mode in either the task type configuration or the orchestration configuration.")
      ...
      task.add(planningInfo.renderMarkdown(true))
      ...
      fun runExecution(): String {
        ...
        cognitiveInstance.handleUserMessage(planningGoal.let { planningGoal ->
          var planningGoal = planningGoal
        ...
        val summary = createSummary(results, planningGoal, agent(task, planningGoal))
        transcript?.write("\n\n## Summary\n\n$summary\n\n".toByteArray())
        return summary
      }
  ...
  private fun createSummary(
    results: List<String>, goal: String, summaryAgent: ChatAgent
  ): String {
    ...
    if (results.isEmpty()) {
      return buildString {
        ...
        appendLine()
        appendLine("The sub-plan executed successfully.")
      }
    }
