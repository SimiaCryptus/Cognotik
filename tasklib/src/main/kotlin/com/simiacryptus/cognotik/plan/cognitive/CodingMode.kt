...
open class CodingMode(
  ...
  inner class TaskFunctionImpl<T : TaskExecutionConfig, U : TaskTypeConfig>(
    ...
    override fun call(executionConfig: Any, message: String): String {
      ...
      try {
        try {
          orchestrationConfig.getImpl(
            taskType as TaskType<T, U>, (executionConfig.jsonCast<Map<String, Any>>() + mapOf(
  ...
  override fun handleUserMessage(userMessage: String, task: SessionTask) {
    ...
    try {
      ...
      tabs["Code"] = ("```" + config.codeRuntime.name.lowercase()
        .replace("runtime", "") + "\n" + response.code + "\n```").renderMarkdown()
      transcript?.write("Code:\n${response.code}\n".toByteArray())
      task.resolveUserFile(".logs/code_${now()}.${config.codeRuntime.extension}")
        ?.writeBytes(response.code.toByteArray())
