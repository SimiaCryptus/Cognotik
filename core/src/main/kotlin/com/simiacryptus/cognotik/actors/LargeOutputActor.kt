    ...
    override fun respond(input: List<String>, api: API, vararg messages: ApiModel.ChatMessage): String {
        ...
        while (iterations < maxIterations) {
            if (accumulatedResponse.isEmpty()) {
                accumulatedResponse = response(input = messages, api = api).choices.first().message?.content?.trim()
            ...
            accumulatedResponse = pairs.reversed().fold(accumulatedResponse) { acc, (match, response) ->
                ...
                if (replacement.startsWith("```")) {
                    replacement =
                        replacement.lines().drop(1).reversed().dropWhile { !it.startsWith("```") }.drop(1).reversed()
                            .joinToString("\n")
                }
                ...
                if (commonPrefix.isNotBlank() && commonPrefix.contains('\n')) replacement =
                    replacement.substring(commonPrefix.length)
                val largestCommonSubstring = largestCommonSubstring(replacement, suffix)
                if (largestCommonSubstring.isNotBlank()) replacement =
                    replacement.substring(0, replacement.indexOf(largestCommonSubstring))
                ...
                repeat(step.iterations) { iteration ->
                    if (step.perSection) {
                        ...
                        sections.forEach { (header, content) ->
                            ...
                            val modifiedSection = diffApplier.apply(
                                content,
                                sectionResponse.choices.first().message?.content ?: ""
                            ).newCode
