package com.simiacryptus.cognotik.docops.exec
    ...
    interface DocTaskKind {
      /** Stable, human readable name (also the `task_type` discriminator in JSON configs). */
      val name: String
    ...
    /** Minimal concurrency abstraction so the core logic does not depend on platform thread pools. */
    ...
    data class DocTaskRequest<K : DocTaskKind>(
      val taskKind: K,
      val message: String,
    ...
    data class DocTaskInferenceRequest<K : DocTaskKind>(
      val taskKind: K,
    ...
    interface DocTaskCallbacks<S : Any> {
      /** May throw [java.util.concurrent.CancellationException] to abort. */
    ...
    interface DocExecutionContext<K : DocTaskKind, S : Any> : AutoCloseable {
      ...
      fun inferTaskConfig(request: DocTaskInferenceRequest<K>): Map<String, Any>

      fun execute(request: DocTaskRequest<K>, callbacks: DocTaskCallbacks<S>)
