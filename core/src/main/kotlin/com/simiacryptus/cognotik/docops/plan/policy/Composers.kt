package com.simiacryptus.cognotik.docops.plan.policy
    ...
    class TaskDescriptionComposer<K : DocTaskKind> {
      ...
      fun compose(contributions: List<TargetContribution>, target: TargetPath, kind: K): String {
        ...
        return buildString {
          when {
            ...
            kind.isSubPlanTask -> {
              appendLine("Perform ${kind.name} generation.")
              appendLine("Use the provided documentation and specifications as context for the processing.")
            }

            !kind.isFileTask -> {
              appendLine("Produce the file ${target.name} according to the task type ${kind.name}.")
              appendLine("Use the provided documentation and specifications as context for the processing.")
            }
