...
open class FuzzyPatchMatcher(
  ...
  override fun applyPatch(source: String, patch: String): String {
    ...
    if (source.isBlank()) {
      val newContent = patch.lines().map { line ->
        when {
          line.startsWith("+") -> line.removePrefix("+").trimStart()
          line.startsWith("-") -> "" // Ignore deletions in this case. Should we warn?
          else -> line.trimStart()
        }
  ...
  private fun isMatch(
    ...
    // Check for code block markers
    val sourceIsCodeBlock = sourcePrev.line?.trimStart()?.startsWith("$TRIPLE_TILDE") ?: false
    val patchIsCodeBlock = patchPrev.line?.trimStart()?.startsWith("$TRIPLE_TILDE") ?: false
