...
export const useWebSocket = (sessionId: string) => {
    ...
    useEffect(() => {
        ...
        if (!sessionId) {
            console.error('[WebSocket] Critical error: No sessionId provided, connection aborted');
            return;
        }
        const handleMessage = (message: Message) => {

            if (!message?.id || !message?.version) {
                console.warn('[WebSocket] Received message with missing id or version:', message);
                return;
            }
