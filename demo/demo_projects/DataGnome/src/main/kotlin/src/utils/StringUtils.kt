...
/**
 * A utility class for comprehensive string manipulation operations.
 */
object StringUtils {
    /**
     * Validates if the provided URL is in a correct format using DataValidationUtils.
     * @param url The URL string to validate.
     * @return True if the URL is valid, false otherwise.
     */
    fun validateURL(url: String): Boolean {
        return DataValidationUtils.isValidURL(url)
    }

    private val operations: MutableMap<String, (String?, Array<out Any>) -> Any> = mutableMapOf()
    ...
    /**
     * Registers a new string operation.
     * @param name The name of the operation.
     * @param operation The operation function.
     */
    fun registerOperation(name: String, operation: (String?, Array<out Any>) -> Any) {
        operations[name] = operation
    }

    /**
     * Executes a registered string operation.
     * @param name The name of the operation.
     * @param str The string to operate on.
     * @param args Additional arguments for the operation.
     * @return The result of the operation.
     */
    fun executeOperation(name: String, str: String?, vararg args: Any): Any? {
        return operations[name]?.invoke(str, args)
    }

    private fun registerDefaultOperations() {
        registerOperation("trim") { str, _ -> trim(str) }
        registerOperation("split") { str, args -> split(str, args[0] as String) }
    ...
    fun toUpperCase(str: String?): String {
        return StringOperations.toUpperCase(str)
    }
