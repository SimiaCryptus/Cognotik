...
export class BrowserAutomation {
   ...
   */
  async launch(sessionConfig: TestSessionConfig): Promise<BrowserSession> {
    try {
      // Select browser based on configuration
      const browserType = this.config.browserType || 'chromium';
      switch (browserType) {
        case 'firefox':
          this.browser = await firefox.launch({ headless: this.config.headless !== false });
          break;
        case 'webkit':
          this.browser = await webkit.launch({ headless: this.config.headless !== false });
          break;
        case 'chromium':
        default:
      ...
      const taskId = Object.keys(sessionConfig.taskSettings).find(
        key => sessionConfig.taskSettings[key].enabled
      );
   ...
   */
  private async saveSessionConfig(sessionConfig: TestSessionConfig): Promise<void> {
    if (!this.page) {
      throw new Error('Browser page not initialized');
    }
   ...
   */
  async downloadGeneratedFiles(targetDir: string): Promise<string[]> {
    if (!this.page) {
      throw new Error('Browser page not initialized');
    }
