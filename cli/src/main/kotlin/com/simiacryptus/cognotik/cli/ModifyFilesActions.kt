...
object ModifyFilesActions {
  ...
  private fun handleModify(ctx: FsActionContext) {
    ...
    SessionProxyServer.agents[session] = PatchChatManager(
      session = session,
      model = models?.smart?.asChatInterface(user ?: throw IllegalStateException("No user configured"))
        ?: throw IllegalStateException("No smart model configured"),
      fastModel = models?.fast?.asChatInterface(user ?: throw IllegalStateException("No user configured"))
        ?: throw IllegalStateException("No fast model configured"),
      root = root,
      files = selected,
