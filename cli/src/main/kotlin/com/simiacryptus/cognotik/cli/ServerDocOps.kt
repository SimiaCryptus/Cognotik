...
object ServerDocOps {
  ...
  class Request(
    ...
    /** The servlet that actually does the work (defaults to the mounted one). */
    val servlet: DocProcessorServlet = FileServerCli.docProcessorServlet
      ?: throw IllegalStateException("the DocOps servlet is not installed"),
  )
  ...
  fun models(user: User): Int {
    println(DocProcessorServlet.describeModels(availableModels(user)))
    return 0
  }
   ...
   * output scraping is involved. Planning is pure, hence safe on a read-only mount.
   */
  ...
  /** Translates the CLI/FS-API shaped [Request] into the servlet's request. */
  private fun docOpsRequest(req: Request): DocProcessorServlet.DocOpsRequest {
    val available = availableModels(req.user)
    val models = DocProcessorServlet.models(
      smartModel = req.smartModel?.takeIf { it.isNotBlank() } ?: throw IllegalArgumentException(
        "no smart model selected. Start the server with --smart-model <id> " +
            "or set COGNOTIK_SMART_MODEL.\n" + DocProcessorServlet.describeModels(available)
      ),
