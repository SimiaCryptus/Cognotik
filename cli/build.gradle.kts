...
tasks.war {
    ...
    manifest {
        attributes(
            "Main-Class" to "com.simiacryptus.cognotik.docops.cli.DocOpsCli"
        )
    }
}
    ...
    doLast {
        ...
        execOperations.exec {
            commandLine(
                ...
                "--icon", iconFile.path,
                "--main-class", "com.simiacryptus.cognotik.docops.cli.DocOpsCli",
...
tasks.register("prepareLinuxDesktopFile") {
    group = "build"
    description = "Copies cli files and icons to the jpackage resource directory for Linux"
    onlyIf { System.getProperty("os.name").lowercase().contains("linux") }
    doLast {
        ...
        // Use the package name for the main cli file.
        val mainDesktopFile = File(resourcesDir, "cognotik.cli")
        ...
        println("Created cli files in jpackage resources directory:")
