...
class CustomFileSetPatchServer(
    ...
    override fun newSession(user: User?, session: Session): SocketManager {
       ...
       _root = config.selectedDirectory ?: config.project?.basePath?.let { Path.of(it) } ?: Path.of(".")
        ...
        val userMessage = config.settings?.transformationMessage ?: "Review and improve the code according to best practices"
        ...
        val contextSummary = if (contextFiles.isNotEmpty()) {
        ...
        val futures = fileSets.map { fileSet ->
            fixedConcurrencyProcessor.submit {
                try {
                    ...
                    val fileSetContent = fileSet.files.joinToString("\n\n") { path ->
                        """
                    ...
                    if (outputMode == CustomFileSetPatchAction.OutputMode.EDIT_FILES && autoApply) {
                    ...
                    } else if (outputMode != CustomFileSetPatchAction.OutputMode.EDIT_FILES) {
                        // Handle documentation/extract generation
                        ...
                        if (config.settings?.singleOutputFile == true) {
                            ...
                            fileTask.complete("<a href='fileIndex/$session/${_root.relativize(outputFile)}'>Generated: ${_root.relativize(outputFile)}</a>")
                        }
                    } else {
                        Discussable(
                            task = fileTask,
        ...
        fixedConcurrencyProcessor.submit {
            ...
            if (outputMode != CustomFileSetPatchAction.OutputMode.EDIT_FILES && config.settings?.singleOutputFile == true) {
                ...
                Files.write(outputFile, sortedContent.toByteArray())
                task.add("<a href='fileIndex/$session/${_root.relativize(outputFile)}'>Generated: ${_root.relativize(outputFile)}</a>")
            }
            
            status.append("All file sets processed successfully.<br/>")
            task.update()
        }
