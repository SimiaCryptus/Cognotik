...
open class DocProcessorAction(
  ...
  private fun run(
    ...
    try {
      ...
      val sessions = mutableListOf<Session>()
      docProcessor.separateQueues(selectedTasks).map { docProcessor.sortByDependencies(it) }.filter { it.isNotEmpty() }
        .map { mods ->
          newProcessor(user = localUser).submit {
            object : UnifiedHarness(
              ...
              mods.forEach { mod ->
                val mod = mod.rebase(
                  docProcessor.root,
                  mod.data.relative_files?.firstOrNull()?.let { docProcessor.root.resolve(it).parentFile }
                    ?: docProcessor.root)
  ...
  class DocProcessorTaskDialog(
    ...
    private fun filterTasks(query: String) {
      ...
      val filteredItems = if (query.isBlank()) {
        ...
        taskItems.filter { item ->
          item.displayName.lowercase().contains(lowerQuery) || item.description.lowercase().contains(lowerQuery)
        }
      }
    ...
    data class TaskItem(
      val index: Int, val displayName: String, val description: String, val config: DocProcessor.ModificationTaskConfig
    ) {
