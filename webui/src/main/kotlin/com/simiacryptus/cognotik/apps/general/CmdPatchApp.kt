...
class CmdPatchApp(
  ...
  override fun codeFiles() = getFiles(files).filter { it.toFile().length() < 1024 * 1024 / 2 }.map { root.toPath().relativize(it) ?: it }.toSet()
  ...
  override fun projectSummary(): String {
    ...
    val str = codeFiles.asSequence().filter { root.toPath().resolve(it).toFile().exists() }.distinct().sorted().joinToString("\n") { path ->
        "* $path - ${
  ...
  override fun searchFiles(searchStrings: List<String>) = searchStrings.flatMap { searchString ->
    ...
    FileSelectionUtils.filteredWalk(settings.workingDirectory!!).filter { it.readText().contains(searchString, ignoreCase = true) }.map { it.toPath() }.toList()
  }.toSet()
}