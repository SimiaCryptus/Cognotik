...
- **Database Modes**: Automatically switches between `jdbc:hsqldb:file` (if a root directory is provided) and
  `jdbc:hsqldb:mem` (if no root is provided).
...
- **Persistence**: Like the metadata storage, it supports both persistent file storage and in-memory operation for
  testing or ephemeral environments.

## Implementation Details

- **Drivers**: Uses the standard HSQLDB JDBC driver (`org.hsqldb.jdbc.JDBCDriver`).
- **Schema Evolution**: Both classes include `createSchema` logic that executes `CREATE TABLE IF NOT EXISTS` during
  initialization, ensuring the database is ready for use without manual setup.
- **Concurrency**: Configured with `hsqldb.lock_file=false` and `shutdown=true` in file mode to manage database locks
  and ensure clean shutdowns within the application lifecycle.