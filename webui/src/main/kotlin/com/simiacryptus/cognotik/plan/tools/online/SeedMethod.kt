...
enum class SeedMethod : SeedMethodFactory {
  ...
  SearchIO_Google_Maps {
    override fun createStrategy(task: CrawlerAgentTask, user: User?): SeedStrategy = SearchAPISearch("google_maps", "local_results").createStrategy(task, user)
  },
  ...
  SearchIO_Google_Jobs {
    override fun createStrategy(task: CrawlerAgentTask, user: User?): SeedStrategy = SearchAPISearch("google_jobs", "jobs").createStrategy(task, user)
  },
  SearchIO_Amazon {
    override fun createStrategy(task: CrawlerAgentTask, user: User?): SeedStrategy =
  ...
  SearchIO_Bing {
    override fun createStrategy(task: CrawlerAgentTask, user: User?): SeedStrategy = SearchAPISearch("bing", "organic_results").createStrategy(task, user)
  },
  SearchIO_DuckDuckGo {
    override fun createStrategy(task: CrawlerAgentTask, user: User?): SeedStrategy = SearchAPISearch("duckduckgo", "organic_results").createStrategy(task, user)
  },
  SearchIO_EBay {
    override fun createStrategy(task: CrawlerAgentTask, user: User?): SeedStrategy =
