  ...
  ** Optionally, list input files (supports glob patterns) to be examined for context
  ...
  override fun run(
    ...
    try {
      ...
      val relaxedSolutionTask = task.ui.newTask(false)
      ...
      constraintsToReintroduce.forEachIndexed { index, (constraint, priority) ->
        ...
        val iterationTask = task.ui.newTask(false)
      ...
      val (detailedLink, detailedFile) = Pair(task.linkTo("constraint_relaxation_detailed.md"), task.resolveUserFile("constraint_relaxation_detailed.md"))
  ...
  private fun generateSynthesis(
    ...
    val prompt = buildString {
      ...
      reintroductionSteps.forEachIndexed { index, step ->
        appendLine("${index + 1}. ${step.constraint} (priority: ${String.format("%.2f", step.priority)}, time: ${step.iterationTime / 1000.0}s)")
      }
      appendLine()
