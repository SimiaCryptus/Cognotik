...
Ensure the dialogue sounds natural when spoken aloud.
        ...
        hookTask.add(
          buildString {
            ...
            appendLine("---")
            appendLine()
            appendLine("**Duration:** ${hookSegment.duration_seconds}s | **Words:** ${hookSegment.dialogue.split("\\s+".toRegex()).size}")
            appendLine()
            appendLine("**Status:** ✅ Complete")
          }.renderMarkdown
        )
...
${if (sectionOutline.visual_suggestions.isNotEmpty()) "Visual Suggestions: ${sectionOutline.visual_suggestions.joinToString("; ")}" else ""}
...
Aim for approximately ${sectionOutline.estimated_duration_seconds} seconds of content.
        ...
        sectionTask.add(
          buildString {
            ...
            appendLine("---")
            appendLine()
            appendLine("**Duration:** ${sectionSegment.duration_seconds}s | **Words:** ${sectionSegment.dialogue.split("\\s+".toRegex()).size}")
            appendLine()
            appendLine("**Status:** ✅ Complete")
          }.renderMarkdown
        )
...
Target duration: 15-20 seconds.
      ...
      closingTask.add(
        buildString {
          ...
          appendLine("---")
          appendLine()
          appendLine("**Duration:** ${closingSegment.duration_seconds}s | **Words:** ${closingSegment.dialogue.split("\\s+".toRegex()).size}")
          appendLine()
          appendLine("**Status:** ✅ Complete")
        }.renderMarkdown
      )
