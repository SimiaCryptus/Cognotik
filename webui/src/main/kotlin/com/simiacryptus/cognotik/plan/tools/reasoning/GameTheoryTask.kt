  ...
  override fun run(
    ...
    try {
      ...
      val structureLoading = structureTask.add(
        MarkdownUtil.renderMarkdown("## Game Structure\n\n🔄 Analyzing game structure and strategies...", ui = ui)
      )
      task.update()
...
Generate the dominant strategy analysis now:
      ...
      if (executionConfig.find_pareto_optimal) {
        ...
        val paretoLoading = paretoTask.add(
          MarkdownUtil.renderMarkdown("## Pareto Optimality\n\n🔄 Identifying Pareto optimal outcomes...", ui = ui)
        )
        task.update()
...
Generate the Pareto optimality analysis now:
      ...
      if (executionConfig.repeated_game_analysis) {
        ...
        val repeatedLoading = repeatedTask.add(
          MarkdownUtil.renderMarkdown("## Repeated Game Analysis\n\n🔄 Analyzing repeated game dynamics...", ui = ui)
        )
        task.update()
...
Generate the repeated game analysis now:
      ...
      if (executionConfig.provide_recommendations) {
        ...
        val recommendLoading = recommendTask.add(
          MarkdownUtil.renderMarkdown("## Strategic Recommendations\n\n🔄 Generating recommendations...", ui = ui)
        )
        task.update()
