...
class SoftwareGraphPlanningTask(
...
) : AbstractTask<SoftwareGraphPlanningTask.GraphBasedPlanningTaskExecutionConfigData, TaskTypeConfig>(
    orchestrationConfig,
    planTask
) {
    ...
    override fun run(
        ...
        markdownTranscript?.write("## Planning Response\n\n".toByteArray())
        markdownTranscript?.write("```yaml\n${response.text}\n```\n\n".toByteArray())
        ...
        markdownTranscript?.write(planSummary.toByteArray())
