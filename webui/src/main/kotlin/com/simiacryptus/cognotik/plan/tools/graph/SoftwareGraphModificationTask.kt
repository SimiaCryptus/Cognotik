...
) : AbstractTask<SoftwareGraphModificationTask.SoftwareGraphModificationTaskExecutionConfigData, TaskTypeConfig>(
    orchestrationConfig,
    planTask
) {
   ...
   """.trimIndent()
    ...
    override fun run(
        ...
        val graphModificationActor = ParsedAgent(
            ...
            """.trimIndent() + SoftwareNodeType.values().joinToString("\n") {
                "* " + it.name + ": " + it.description?.prependIndent("  ") +
                        "\n    " + TaskContextYamlDescriber(orchestrationConfig).describe(rawType = it.nodeClass)
                    .lineSequence()
                    .map {
                        when {
                            it.isBlank() -> {
        ...
        val inputFile = (orchestrationConfig.absoluteWorkingDir?.let { File(it) } ?: File("."))
            .resolve(
                executionConfig?.input_graph_file ?: throw IllegalArgumentException("Input graph file not specified")
            )
        ...
        transcript?.write(
            "- Node types: ${
                originalGraph.nodes.groupBy { it.javaClass.simpleName }.map { "${it.key}: ${it.value.size}" }
                    .joinToString(", ")
            }\n\n".toByteArray()
        )
