...
class CommandSessionTask(
  ...
  class CommandSessionTaskExecutionConfigData(
    ...
    task_type = CommandSession.name, task_description = task_description, task_dependencies = task_dependencies?.toMutableList(), state = state
  )
  ...
  override fun run(
    ...
    try {
      ...
      process = executionConfig.sessionId?.let { id -> activeSessions[id] } ?: ProcessBuilder(executionConfig.command).redirectErrorStream(true).start()
        .also { newProcess ->
