...
open class DocProcessor(
  ...
  fun getAll(
    ...
    // Expand all documents specs - the document itself is the target
    // and matching files are the supporting context
    data class DocumentMatch(
      val docSpec: DocSpec,
      val supportingFiles: List<File>
    )

    val documentMatches: List<DocumentMatch> = docSpecs
      .filter { it.documents.isNotEmpty() }
      .map { spec ->
        val baseDir = spec.docFile.parentFile
        val supportingFiles = spec.documents.flatMap { pattern ->
          val matchedFiles = expandGlobPattern(baseDir, pattern)
    ...
    // Group document matches by the document file (target)
    val fileToDocuments = documentMatches.groupBy { it.docSpec.docFile.absolutePath }
    ...
    val allTargetFiles = (fileToSpecs.keys + fileToTransforms.keys + fileToDocuments.keys).distinct()
   ...
   */
  private fun parseDocuments(frontmatter: Map<String, Any>): List<String> {
