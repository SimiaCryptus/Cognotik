...
The `RobotsTxtParser` class is responsible for fetching, parsing, and caching `robots.txt` files. It ensures that the
crawler respects the directives set by website administrators.
...
* **Pattern Matching**: Supports standard `robots.txt` pattern matching, including wildcards (`*`) and end-of-string
  anchors (`$`).
* **Bot Identification**: Identifies as `CognotikBot/1.0` and prioritizes rules specific to this agent or the global
  wildcard (`*`).
