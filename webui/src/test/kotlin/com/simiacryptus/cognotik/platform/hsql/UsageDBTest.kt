...
abstract class UsageTest(private val impl: UsageInterface) {
  companion object {
    ...
    val tempDBDir: String by lazy { newTempDir() }

    private fun newTempDir(): String {
      val tempDir = System.getProperty("java.io.tmpdir")
      val uniqueDir = "$tempDir/cognotik_test_${System.currentTimeMillis()}_${Random.nextInt()}"
      val dir = File(uniqueDir)
      if (!dir.exists()) {
        dir.mkdirs()
      }
      log.info("Created temporary database directory: {}", uniqueDir)
      return uniqueDir
    }
