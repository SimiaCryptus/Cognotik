...
This package contains a collection of utility classes and functions used across the Cognotik WebUI for HTML processing,
image manipulation, markdown rendering, and web automation.
...
A simple concurrency utility that wraps a `Consumer` to ensure that only one execution happens at a time. If the
consumer is already busy, subsequent calls are ignored until the current execution completes.
