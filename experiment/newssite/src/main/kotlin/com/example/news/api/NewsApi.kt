...
data class Story(
  @Description("The unique identifier for the story.") val id: Long = 0,
  ...
  @Description("A list of unique IDs for the top-level comments on this story.") val commentIds: List<Long> = emptyList(),
  ...
  init {
    require(id > 0) { "Story ID must be positive" }
    require(title.isNotBlank()) { "Story title cannot be blank" }
    require(author.isNotBlank()) { "Story author cannot be blank" }
    require(score >= 0) { "Story score cannot be negative" }
    require(commentCount >= 0) { "Comment count cannot be negative" }
    require(url != null || text != null) { "Story must have either a URL or text content" }
  }
}

@Description("Represents a comment made on a story or another comment.")
data class Comment(
  @Description("The unique identifier for the comment.") val id: Long = 0,
  ...
  @Description("The ID of the parent item (either a story or another comment).") val parentId: Long = 0,
  @Description("A list of unique IDs for direct replies to this comment.") val childCommentIds: List<Long> = emptyList(),
...
data class User(
  ...
  @Description("A list of unique IDs for items (stories and comments) submitted by the user.") val submittedIds: List<Long> = emptyList(),
  ...
  init {
    ...
    require(karma >= 0) { "Karma cannot be negative" }
  }
}
...
data class PaginatedStoryIds(
  @Description("A list of unique IDs for stories.") val ids: List<Long> = emptyList(),
...
interface NewsApiService {
  ...
  fun getStoryDetails(id: Long): Story

  @Description("Fetches the full details for a given comment.")
  fun getCommentDetails(id: Long): Comment
  ...
  fun getStoryComments(
    storyId: Long, pagination: PaginationParams = PaginationParams()
  ): PaginatedCommentIds
...
data class PaginatedCommentIds(
  @Description("A list of unique IDs for comments.") val ids: List<Long> = emptyList(),
  @Description("The current page number.") val page: Int,
  @Description("The number of items per page.") val pageSize: Int = 30,
