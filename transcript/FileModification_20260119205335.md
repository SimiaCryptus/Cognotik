...
Here is the updated `README.md` for the `com.simiacryptus.cognotik.webui.session` (and associated `chat`) package, based on the provided source code and specifications.

### webui/src/main/kotlin/com/simiacryptus/cognotik/webui/session/README.md
```markdown
# Session and Chat Management

This package provides the core infrastructure for building interactive, LLM-powered chat applications. It handles WebSocket communication, session state persistence, complex query expansion, and intelligent model routing.

## Key Components

### Application Infrastructure
*   **`BasicChatApp`**: A concrete implementation of `ApplicationServer` that sets up a chat environment. It manages model selection (smart vs. fast), user settings, and session initialization.
*   **`ChatServer`**: An abstract base class for Jetty-based WebSocket servers. It handles servlet registration (`/ws`, `/newSession`) and maps sessions to `SocketManager` instances.
*   **`ChatSocket`**: The WebSocket adapter that bridges the network connection to the session logic, handling message replay upon reconnection and traffic logging.

### Session Logic
*   **`SocketManager`**: The base class for managing session state. It handles message versioning, persistence via `StorageInterface`, and command processing (e.g., link clicks, text input).
*   **`ChatSocketManager`**: Extends `SocketManager` to provide chat-specific features. It maintains the conversation history and implements the query expansion engine.
*   **`SmartChatSocketManager`**: An advanced manager that adds:
    *   **History Compaction**: Automatically summarizes older parts of the conversation when token limits are reached to maintain context within model windows.
    *   **Query Elevation**: Uses a fast model to analyze user queries and decide if they require the reasoning capabilities of a "smart" model.

### Task Management
*   **`SessionTask`**: Represents a specific unit of work or a message thread within a session. It provides a rich API for UI generation:
    *   `echo`/`add`/`complete`: Standard message output with Markdown support.
    *   `error`: Formatted error display with stack traces.
    *   `expandable`/`expanded`: Collapsible UI sections.
    *   `saveFile`/`image`: Resource management and file linking.
    *   `newLogStream`: Integrated API logging for debugging LLM interactions.

## Features

### Query Expansion Syntax
The `ChatSocketManager` supports a powerful expansion syntax to automate complex prompting workflows:

| Syntax | Type | Description | Example |
| :--- | :--- | :--- | :--- |
| `@[opt1\|opt2]` | **Parallel** | Runs the prompt for each option in parallel. | `Joke about @[cats\|dogs]` |
| `@{a -> b}` | **Sequence** | Chains prompts where output of *a* feeds into *b*. | `@{Summarize -> Translate}` |
| `@(1..5:1)` | **Range** | Iterates over a numeric range. | `Events in year @(1900..1910:2)` |
| `@TopicName` | **Reference** | References previously extracted entities. | `Tell me more about @Person` |

### Topic Extraction
The system automatically identifies named entities and topics in assistant responses using a secondary "fast" model. These topics are aggregated and can be referenced in subsequent user messages using the `@Topic` syntax.

### Intelligent Routing and Summarization
`SmartChatSocketManager` optimizes for both cost and performance:
1.  **Elevation**: Simple queries (greetings, facts) stay on cheaper, faster models. Complex reasoning (coding, math) is elevated to high-capability models.
2.  **Compaction**: When the conversation exceeds `maxHistoryTokens` (default 4000), it generates a concise summary of older messages, preserving only the most recent context to save tokens and maintain performance.

## Usage Patterns

### Creating a Chat App
```kotlin
class MyCustomChat(root: File, model: ChatModel) : BasicChatApp(
    root = root,
    model = model,
    parsingModel = Models.FAST_MODEL,
    applicationName = "My Assistant"
)
```

### Interacting with the UI from a Task
```kotlin
task.header("Processing Data", level = 2)
task.verbose("Starting complex calculation...")
val fileUrl = task.saveFile("data.csv", csvBytes)
task.complete("Analysis finished. Download here: [Data]($fileUrl)")
```

## Implementation Details
*   **Concurrency**: Uses `FixedConcurrencyProcessor` for parallel expansions and a dedicated thread pool per session.
*   **Persistence**: Message states are synchronized with `StorageInterface` to allow session recovery across server restarts.
*   **Rendering**: Supports GitHub-flavored Markdown and custom HTML components (spinners, cancel buttons, elevation notices).
```

### Summary of Changes
- Created a new `README.md` in the `webui/src/main/kotlin/com/simiacryptus/cognotik/webui/session/` directory.
- Documented the primary classes: `BasicChatApp`, `ChatServer`, `ChatSocket`, `SocketManager`, `ChatSocketManager`, `SmartChatSocketManager`, and `SessionTask`.
- Detailed the "Query Expansion Syntax" which is a significant feature of the `ChatSocketManager`.
- Explained the "Smart" features like history compaction and query elevation found in `SmartChatSocketManager`.
- Provided a high-level overview of how the session management system integrates with LLMs and the web UI.
- Followed the project's documentation patterns as specified in the `index_docs.md` transform rules.
</details>
...
java.lang.InterruptedException
	...
	at com.simiacryptus.cognotik.plan.tools.file.FileModificationTask.run(FileModificationTask.kt:203)
	at com.simiacryptus.cognotik.apps.SingleTaskApp.executeTask(SingleTaskApp.kt:111)
