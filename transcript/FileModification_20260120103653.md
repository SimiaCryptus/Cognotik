...
### File Context
# /home/andrew/code/Cognotik/core/src/main/kotlin/com/simiacryptus/cognotik/diff/DiffUtil.kt
    ...
    fun generateDiff(original: List<String>, modified: List<String>): List<PatchLine> {
        ...
        while (i < originalLines.size && j < modifiedLines.size) {
            ...
            if (originalLine == modifiedLine) {
                ...
                val originalIndex = originalLines.subList(i, originalLines.size).indexOf(modifiedLine)
                    .let { if (it == -1) null else it + i }
                val modifiedIndex = modifiedLines.subList(j, modifiedLines.size).indexOf(originalLine)
                    .let { if (it == -1) null else it + j }
...
# /home/andrew/code/Cognotik/core/src/main/kotlin/com/simiacryptus/cognotik/diff/FullReplacementProcessor.kt
...
# /home/andrew/code/Cognotik/core/src/main/kotlin/com/simiacryptus/cognotik/diff/FuzzyPatchMatcher.kt
...
open class FuzzyPatchMatcher(
        ...
        while (i < diff.size - 1) {
            if (diff[i].type == DELETE) {
                ...
                while (j < diff.size && diff[j].type != CONTEXT) {
                    if (diff[j].type == ADD && diff[i].index != -1 && diff[j].index != -1 && normalizeLine(
                            diff[i].line ?: ""
    ...
    private fun linkUniqueMatchingLines(sourceLines: List<LineRecord>, patchLines: List<LineRecord>): Int {
        ...
        val sourceLineMap =
            sourceLines.filter { it.line != null && it.matchingLine == null }.groupBy { normalizeLine(it.line!!) }
    ...
    private fun applySnippetPatch(source: String, patch: String): String {
        ...
        for (i in 0..normalizedSource.size - patchSize) {
            ...
            val hasAnchorMatch =
                normalizedSource[i] == normalizedPatch[0] || normalizedSource[i + patchSize - 1] == normalizedPatch[patchSize - 1]
...
# /home/andrew/code/Cognotik/core/src/main/kotlin/com/simiacryptus/cognotik/diff/PatchProcessor.kt
...
# /home/andrew/code/Cognotik/core/src/main/kotlin/com/simiacryptus/cognotik/diff/PatchProcessors.kt
...
# /home/andrew/code/Cognotik/core/src/main/kotlin/com/simiacryptus/cognotik/diff/PatchResult.kt
...
# /home/andrew/code/Cognotik/core/src/main/kotlin/com/simiacryptus/cognotik/diff/PythonPatcher.kt
...
# /home/andrew/code/Cognotik/core/src/main/kotlin/com/simiacryptus/cognotik/diff/SimpleDiffApplier.kt
...
# /home/andrew/code/Cognotik/core/src/main/kotlin/com/simiacryptus/cognotik/diff/ThermodynamicPatchMatcher.kt
...
Here is the documentation for the `com.simiacryptus.cognotik.diff` package.

### core/src/main/kotlin/com/simiacryptus/cognotik/diff/README.md
```markdown
...
### Summary of Changes
- Created a new `README.md` for the `com.simiacryptus.cognotik.diff` package.
- Documented the purpose of the package and its focus on LLM-generated patches.
- Detailed the key components: `PatchProcessor`, `FuzzyPatchMatcher`, `ThermodynamicPatchMatcher`, `PythonPatcher`, `PatchProcessors`, and `SimpleDiffApplier`.
- Provided usage examples for basic patching and applying patches from LLM responses with validation.
- Listed configuration parameters for the advanced matchers.
</details>

                - <a href='fileIndex/G-20260120-RPFC/core/src/main/kotlin/com/simiacryptus/cognotik/diff/README.md'>core/src/main/kotlin/com/simiacryptus/cognotik/diff/README.md</a> Updated
